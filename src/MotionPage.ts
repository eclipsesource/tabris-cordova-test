...
export default class MotionPage extends PluginPage {
  ...
  private _startAccelerationWatch() {
    let onSuccess = acceleration =>
      this.find('#accelerationDisplay').only(TextView).set({text: this._accelerationToString(acceleration)});
