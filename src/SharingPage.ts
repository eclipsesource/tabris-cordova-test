...
import {Button, Composite, ScrollView, TextInput, TextView} from 'tabris';
...
export default class SharingPage extends PluginPage {
  ...
  protected createUI() {
    ...
    this.content.append(
      ...
      new TextInput({id: 'input', message: INPUT_MESSAGE})
        .onInput(({text: message}) => this.find(SharingSection).forEach(section => section.message = message)),
  ...
  protected applyStyles() {
    super.applyStyles();
    this.find('#textToShareLabel').only(TextView).set(
      {font: '20px'}
    );
  }
...
let sharingSectionConfigurations = [{
  ...
  options: [
    // When using shareVia, make sure the share option is available with window.plugins.socialsharing.canShareVia(..)
    {
      ...
      name: 'Share via SMS',
      handler: message => window.plugins.socialsharing.shareViaSMS(message,
...
function shareSuccessful(success) {
  if (success) {
    (plugins as any).toast.showShortCenter('Thank you for sharing :)');
    console.log('Success sharing: ' + success);
  } else {
    (plugins as any).toast.showShortCenter('Did you cancel? Not awesome enough for you?');
    console.log('Cancelled sharing. Response: ' + success);
  }
}

function shareError(errormsg) {
  (plugins as any).toast.showShortCenter('Error sharing: ' + errormsg + '. Do you have this feature available?');
